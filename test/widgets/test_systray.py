...
def test_no_duplicates_multiple_instances(manager_nospawn, backend_name):
    ...
    manager_nospawn.start(duplicates_config)
...
def test_no_duplicates_mirror(manager_nospawn, backend_name):
...
def reconfigure_screens_config():
    class Conf(MinimalConf):
        screens = [libqtile.config.Screen(top=libqtile.bar.Bar([widget.Systray()], 10))]

    return Conf()


def test_systray_reconfigure_screens(manager_nospawn, backend_name):
    ...
    manager_nospawn.start(reconfigure_screens_config)
...
def icons_config():
    class Conf(MinimalConf):
        screens = [libqtile.config.Screen(top=libqtile.bar.Bar([widget.Systray()], 40))]

    return Conf()


def test_systray_icons(manager_nospawn, backend_name):
    ...
    manager_nospawn.start(icons_config)
